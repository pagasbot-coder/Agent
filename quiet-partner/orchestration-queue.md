...
**Последнее обновление:** 2026-08-10 (эпик Проект МТС + T-Brand `/brand` на одной ветке)
...
**AC:**
- [x] Вычитать `prd-focus-today.md` **на русском**: ясные предложения, TL;DR, иерархия
- [x] Факты и AC не менять
- [x] Режим «Для команды»

#### T-101 — Мобильные карточки реестров (UI/UX + Developer)

**AC:**
- [x] Длинный текст: textarea + wrap (desktop table)
- [x] На узком экране: карточки полей вместо горизонтального скролла таблицы
- [x] Desktop `md+`: прежняя таблица
...
| T-116 | Сохранить статус реестра и обновить радар одной кнопкой | Developer + DevOps | DONE | P0 | T-107 | `@components/stages/StagesShell.tsx` `@lib/stages/stagesStore.ts` | Prod: PostgreSQL; статус переживает переход Пульт → радар → Пульт; исходные данные восстановлены |
...
| 2026-09-10 | **T-116 deploy retry:** причина найдена — Hobby collaboration проверяет commit author; новый deploy-коммит подписывается email владельца Vercel `pagasbot@gmail.com`. |
| 2026-09-10 | **T-116 DB fix:** первый успешный deploy выявил отсутствие `stages_projects`; добавлена безопасная `CREATE TABLE IF NOT EXISTS` при первом PostgreSQL-запросе; lint/build PASS. |
| 2026-09-10 | **T-116 DONE:** production `backend=postgres`; browser smoke «устарел» → сохранить → радар → Пульт PASS; тестовая строка возвращена в исходный статус «открыт». |
