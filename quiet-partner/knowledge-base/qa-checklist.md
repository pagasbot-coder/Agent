...
| BR3 | CTA | «Сохранить и обновить радар» на `/stages`; сначала успешный PUT, затем переход |
...
| BR9 | Закрытие неизвестного | `закрыт` без «Закрыто чем» показывает ошибку и не отправляется на сервер/радар |
| BR10 | D8 preview | До сохранения видно `текущий D8 → новый D8` и разницу |

**Smoke:** на `/stages` есть «Сохранить и обновить радар»; после изменения
статуса и нажатия данные остаются после reload, а `/radar` показывает новую
оценку. Для закрытого вопроса сначала заполнено «Закрыто чем»; chunk содержит
строку баннера.
