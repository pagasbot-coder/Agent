...
**Статус:** DoR закрыт (T-090) · Delivery  
...
- На проде уже есть **Mode Hub**: пульт этапов (`/stages`) и радар (`/radar`), но не было общего артефакта «что делать сегодня» — добавляем карточку **фокуса дня**.
...
- **Не путать** с уже существующим «Фокус недели» (`FocusWeekCard` / `focusWeek`) — это недельный вопрос по слабому домену на `/radar`.

**Статус DoR (T-090):** закрыт 2026-07-22 — live Mode Hub сверен; out of scope и метрики ниже; очередь T-091…T-101 согласована.
