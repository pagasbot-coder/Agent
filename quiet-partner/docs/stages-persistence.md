...
3. Кнопка **«Сохранить и обновить радар»** подтверждает запись и применяет оценку к радару.
4. Кнопка **«Сбросить шаблон МТС»** — только если нужно вернуть канон из кода.

Браузерный `localStorage` остаётся кэшем; источник правды — сервер.
На Vercel файловый backend неустойчив между запросами, поэтому при наличии
`DATABASE_URL` Пульт автоматически выбирает PostgreSQL.

## Env

```bash
STAGES_BACKEND=              # auto: postgres с DATABASE_URL, иначе file
# STAGES_BACKEND=file        # принудительный локальный файл
